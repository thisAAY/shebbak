...
fn main() -> anyhow::Result<()> {
    ...
    if !permissions::check_screen_recording() {
        ...
        eprintln!("Grant it in System Settings > Privacy & Security > Screen Recording, then re-run.");
        std::process::exit(1);
    }
    ...
    let entry = match arg.and_then(|s| s.parse::<u32>().ok()) {
        Some(id) => windows.into_iter().find(|w| w.info.id == id).expect("window id not found"),
        None => {
    ...
    println!("capturing '{}' ({}) at {}x{}px, 5 seconds...", entry.info.title, entry.info.id, w, h);
    ...
    cap.start(Box::new(move |frame| {
        ...
        if c2.load(Ordering::Relaxed) % 30 == 0 {
            let sum: u64 = frame.data[..frame.width as usize * 4].iter().map(|&b| b as u64).sum();
            println!("frame {}: first-row checksum {}", c2.load(Ordering::Relaxed), sum);
        }
    }))?;
    ...
    println!("captured {} frames in 5s (~{} fps), frame size {} bytes", n, n / 5, bytes.load(Ordering::Relaxed));
    if n == 0 {
