...
pub fn chunk_blit(window_id: WindowId, seq: u32, png: &[u8]) -> Vec<HostMessage> {
    let chunks: Vec<&[u8]> = if png.is_empty() {
        vec![&[]]
    } else {
        png.chunks(BLIT_CHUNK_BYTES).collect()
    };
    let count = chunks.len() as u16;
    chunks
        .into_iter()
        .enumerate()
        .map(|(i, c)| HostMessage::TransientBlit {
            window_id,
            seq,
...
impl BlitAssembler {
    ...
    pub fn push(&mut self, msg: &HostMessage) -> Option<(WindowId, Vec<u8>)> {
        ...
        if *seq > entry.seq {
            // newer image supersedes partial
            *entry = Partial {
                seq: *seq,
                chunk_count: *chunk_count,
                received: HashMap::new(),
        ...
        if entry.received.len() as u16 == entry.chunk_count {
            ...
            let mut out = Vec::new();
            for i in 0..count {
                out.extend_from_slice(&entry.received[&i]);
            }
...
impl Default for BlitAssembler {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn forget_lets_a_recycled_window_id_start_fresh() {
        ...
        assert_eq!(
            asm.push(&fresh[0]),
            Some((9, fresh_payload)),
            "seq 1 must assemble after forget"
        );
    }
